<?php
/**
 * Part of WINP\JsonMapper\Mapper
 *
 * PHP version 5
 *
 * @author   Christian Weiske <cweiske@cweiske.de>
 * @package  WINP\JsonMapper\Mapper
...
class Mapper {
	...
	/**
	 * Override class names that WINP\JsonMapper\Mapper uses to create objects.
	...
	public function map( $json, $object ) {
		if ( $this->bEnforceMapType && ! is_object( $json ) ) {
			throw new InvalidArgumentException( 'WINP\JsonMapper\Mapper::map() requires first argument to be an object' . ', ' . gettype( $json ) . ' given.' );
		}
		if ( ! is_object( $object ) ) {
			throw new InvalidArgumentException( 'WINP\JsonMapper\Mapper::map() requires second argument to be an object' . ', ' . gettype( $object ) . ' given.' );
		}
		...
		foreach ( $json as $key => $jvalue ) {
			...
			if ( ! $hasProperty ) {
				if ( $this->bExceptionOnUndefinedProperty ) {
					...
					$this->log( 'info', 'Property {property} does not exist in {class}', [
						'property' => $key,
						'class'    => $strClassName
					] );
				}
			...
			if ( $accessor === null ) {
				...
				$this->log( 'info', 'Property {property} has no public setter method in {class}', [
					'property' => $key,
					'class'    => $strClassName
				] );
				continue;
