	...
	if($secondline_psb_subscribe_entries != '') {
		...
		foreach ( (array) $secondline_psb_subscribe_entries as $key => $entry ) {
			...
			if(($secondline_psb_link != '') && ($secondline_psb_platform_slt != '') && ($secondline_psb_platform_slt != 'custom') ) {
				echo '<span class="secondline-psb-subscribe-'.esc_attr($secondline_psb_platform_slt).'"><a title="' . esc_attr($secondline_psb_platform_slt) . '" onClick="window.open(`'. esc_url($secondline_psb_link) .'`, `_blank`)" onMouseOver="this.style.color=`'. esc_attr(get_post_meta( get_the_ID(), 'secondline_psb_text_color_hover', true )) .'`; this.style.backgroundColor=`'. esc_attr(get_post_meta( get_the_ID(), 'secondline_psb_background_color_hover', true )) .'`" onMouseOut="this.style.color=`'. esc_attr(get_post_meta( get_the_ID(), 'secondline_psb_text_color', true )) .'`; this.style.backgroundColor=`'. esc_attr(get_post_meta( get_the_ID(), 'secondline_psb_background_color', true )) .'`" style="color:'. esc_attr(get_post_meta( get_the_ID(), 'secondline_psb_text_color', true )) .'; background-color:'. esc_attr(get_post_meta( get_the_ID(), 'secondline_psb_background_color', true )) .'" class="button podcast-subscribe-button" href="' . esc_url($secondline_psb_link) . '" target="_blank"><img class="secondline-psb-subscribe-img" src="'. SECONDLINE_PSB_SUBSCRIBE_ELEMENTS_URL .'assets/img/icons/' . esc_attr($secondline_psb_platform_slt) . '.png' .'" /></a></span>';
